...
export default {
  data() {
    return {
      ...
      config: {
        theme: 'snow',
        readOnly: ((this.$cookies.get('token') && this.$cookies.get('token').length) > 0 ? false : true),
        "modules": {
          "toolbar": (this.$cookies.get('token') && this.$cookies.get('token').length) > 0
        }
      }
    }
  },
  ...
  watch: {
    tagsArray: function() {
      ...
      if (this.$cookies.get('token') && this.$cookies.get('token').length) {
        this.saveTags();
      }
    },
    '$route': {
  ...
  methods: {
    initPage: function() {
      ...
      var vm = this;
      axios.defaults.headers.common['Authorization'] = vm.$cookies.get("token");
      ...
      // var post = vm.$cookies.get("editpost");
        ...
        if (document.getElementById('postId').value.length == 0) {
          ...
          switch (deploymentTarget) {
            case LOCALHOST:
              axios.post('/v1/posts', postData)
                .then(function(response) {
                  ...
                  vm.$cookies.set("page", response.data.post);
            ...
            case FBASE:
              ...
              postData.id = uuidv4();
              postData.createdBy = vm.$cookies.getJSON('user').id;
        ...
        switch (deploymentTarget) {
          case LOCALHOST:
            axios.put('/v1/posts/' + vm.id, postData)
              .then(function(response) {
                console.log(response);
                vm.$cookies.set("post", response.data.post);
              })
              .catch(function(error) {
                console.log(error);
              });
      ...
      if (tagText.length && document.getElementById("postId").value.length) {
        ...
        switch (deploymentTarget) {
          ...
          case FBASE:
            ...
            const uuidv4 = require('uuid/v4');
            tagData.id = uuidv4();
            tagData.createdBy = this.$cookies.getJSON('user').id;
