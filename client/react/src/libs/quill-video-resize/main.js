...
class VideoBuilder {
  buildIFrame(src, node) {
    ...
    iframe.setAttribute('width', node.getAttribute('width') || '100%');
  ...
  buildOverlay() {
    ...
    overlay.addEventListener('click', event => {
      const rootElement = getClosest(overlay, '.ql-editor');
      if (rootElement && rootElement.quill) {
        const node = Parchment.find(overlay.parentElement.parentElement);
  ...
  handleMousedown(event) {
    ...
    this.preDragWidth = parseInt(this.iframe.width, 10) || '100%';
    this.preDragHeight = parseInt(this.iframe.height, 10) || 400;
...
class Video extends BlockEmbed {
  static create(src) {
    ...
    const overlay = node.builder.buildOverlay();
